...
class ViewController: UIViewController
{
    ...
    let items = ["CIDissolveTransition", "CIBarsSwipeTransition",
                 "CIModTransition", "CISwipeTransition",
                 "CICopyMachineTransition", "CIFlashTransition", "CIRippleTransition",
                 "BlurTransition", "CircleTransition", "StarTransition"]
    //items.sort()
    ...
    func step()
    {
        ...
        let transformFilterOne = CIFilter(name: "CIAffineTransform",
                                          withInputParameters: [kCIInputImageKey: imageOne,
                                                                kCIInputTransformKey: ViewController.centerImageTransform(image: imageOne)])!
        
        
        let transformFilterTwo = CIFilter(name: "CIAffineTransform",
                                          withInputParameters: [kCIInputImageKey: imageTwo,
                                                                kCIInputTransformKey: ViewController.centerImageTransform(image: imageTwo)])!
    ...
    static func getAllAssets() -> [PHAsset]
    {
        ...
        for index in 0 ..< assetCollections.count
        {
            ...
            let range = NSIndexSet(indexesIn:NSRange(0 ..< assetsInCollection.count))
            
            if let assetsArray = assetsInCollection.objects(at: range as IndexSet) as? [PHAsset]
            {
    ...
    func imageRequestResultHandler(image: UIImage?, properties: [AnyHashable : Any]?)
    {
        ...
        let imageResult = CIImage(image: image)?.applyingOrientation(imageOrientationToTiffOrientation(value: image.imageOrientation))
